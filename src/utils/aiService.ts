...
const cleanDialogueContent = (content: string): string => {
  return content
    // 移除完整的对话性句子（包含表情符号的鼓励语）
    .replace(/^.*?[你我][的]?[看法想法说得理解].*?[很非常]*[好棒深刻正确对].*?[！!].*?[👍😊✨💡🎯].*?\s*/gm, '')
    .replace(/^.*?[太很非常]好[了的]?[！!].*?[👍😊✨💡🎯].*?\s*/gm, '')
    
    // 移除过渡和总结性语句
    .replace(/^.*?现在.*?[我们就].*?[总结看来].*?[：:]/gm, '')
    .replace(/^.*?为了让.*?更好地.*?[理解学习].*?[，。]/gm, '')
    .replace(/^.*?[我们来].*?做.*?[练习总结]/gm, '')
    
    // 移除疑问句（保留知识性问句，移除互动性问句）
    .replace(/^.*?你觉得.*?[？?].*$/gm, '')
    .replace(/^.*?你认为.*?[？?].*$/gm, '')
    .replace(/^.*?[是吗对吧怎么样][？?].*$/gm, '')
    .replace(/^.*?明白了吗[？?].*$/gm, '')
    ...
    // 移除引导性语句
    .replace(/^.*?[太好既然].*?[那么现在就].*?[看学习分析]/gm, '')
    .replace(/^.*?[接下来下面].*?[我们就来]/gm, '')
    
    // 移除表情符号但保留文字
    .replace(/[😊😄😆🤔💡👍📚✨🎯🚀🔧🎨]/g, '')
    
    // 移除常见的对话开头
    .replace(/^(好的|那么|现在|我们现在|接下来|让我们)[，。：]*\s*/gm, '')
    .replace(/^(我们来|我们开始|开始|来看)[学习讲解分析探讨了解看]*[一下]?[，。：]*\s*/gm, '')
