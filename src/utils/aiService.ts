...
请直接返回提炼后的纯知识内容，不要任何其他说明：`;
    ...
    let purifiedContent = response.content?.trim() || '';
    
    // 如果AI返回了说明性文字，尝试提取纯内容
    if (purifiedContent.includes('提炼后的内容：') || purifiedContent.includes('知识内容：')) {
      const contentMatch = purifiedContent.match(/(?:提炼后的内容：|知识内容：)\s*([\s\S]*)/);
      if (contentMatch) {
        purifiedContent = contentMatch[1].trim();
      }
    }
